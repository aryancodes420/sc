...
`assets/motion.js` + the `motion` block in `site.css`: scroll reveals with stagger, header that compacts on scroll with a reading-progress bar, hero that tilts to the mouse with a breathing glow and drifting emoji, category rings with a spinning gradient on hover, product cards that swap to their second photo and lift, fly-to-cart with a cart bump, button ripples, heart pop, accordions that animate open, gallery swipe on touch / magnify on hover / arrow keys, quiz steps that slide, cross-page fade (View Transitions). Everything is transform/opacity only and switches off under `prefers-reduced-motion`.
...
| `product.html` | `?id=<id>` — breadcrumbs, six-photo gallery with competition + stock pills on the photo, three expanding objection cards (fit with a mini size finder that selects the size / real delivery dates + table / quality with material, care, returns) plus keep-it-on, rating line (honest empty state), price + Save % + "You've saved £X" + Pay-in-3 line over £30, three ticks, size picker, express checkout row, trust row, payment logos, sticky add-to-cart, "This is for you if…", videos slot, reviews (score summary, star bars, verified cards, show-all — empty until real), three accordions, cross-sell with the saving, competition block, the quiz, related, recently viewed. Emits Product + BreadcrumbList JSON-LD |
...
| Phone product page | One-row 64px header with a menu button (`mountBurger()`), scroll-snap gallery with dots (video in slot 1 when a clip exists), three objection tiles, name → tiles → buy box → description order, safety & wear block, size table with chest/weight and inches ("maker's chart"), one-line empty slots, `srcset` from `img/400/` (`scripts/build-images.py`), analytics hooks that load nothing until `ANALYTICS` has ids |
...
On every page: a red LED "Sale ends in" ticker to the launch-offer deadline (`SALE.ends`; sits at zero after it — `SALE.active: false` removes the ticker and the strike-throughs, and prices are only ever changed by hand), a genuine Halloween countdown in the top bar (order-by 14 Oct, gone after the dates pass), a "10% off your first order" tab with a one-time pop-up (code `WELCOME10`, demo until the discount exists on Shopify) and a "Help me choose" chat button (WhatsApp when `CONTACT.whatsapp` is set, otherwise the contact page).
