...
| Phone product page | `main-product.liquid`, `header.liquid`, `catwalk.js` | One-row header with a menu button; snap gallery track (first product video in slot 1); subtitle, safety & wear and size-chart metafields; `image_tag` srcset on cards; empty sections collapse to one line; UTMs written to cart attributes |
